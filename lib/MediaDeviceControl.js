...
class MediaDeviceControl {
  ...
  async #getDeviceDescription() {
    try {
      ...
      console.error('无法获取设备描述文件:', error);
      throw error;
    }
  }
  ...
  async #getServiceDetails(service) {
    ...
    try {
      ...
      console.error(`获取服务描述文件失败: ${scpdUrl}`, error);
      throw error;
    }
  }
    ...
    class ServiceClass {
      ...
      async callAction(actionName, params = {}) {
        ...
        try {
          ...
          console.error(`调用服务 ${this.serviceType} 的操作 ${actionName} 失败:`, error);
          throw error;
        }
      }
    }
