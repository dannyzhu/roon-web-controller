...
class MediaDeviceManager extends EventEmitter {
    ...
    async init() {
        logger.info('MediaDeviceManager init');
    ...
    async start() {
        logger.info('MediaDeviceManager start');
        await this.discover.discover();
    }

    async stop() {
        logger.info('MediaDeviceManager stop');
...
class MediaDevice {
    ...
    update(device) {
        logger.info('MediaDevice update : ', this.device.usn);
        this.device = device;
    ...
    async stop() {
        logger.info('MediaDevice stop : ', this.device.usn);
        clearInterval(this.intervalPlay);
        clearInterval(this.intervalDevice);
    }

    printInfo() {
        logger.info('device:', this.device);
        logger.info('deviceInfo:', this.deviceInfo);
    ...
    async setToggleShuffle() {
        logger.info('setToggleShuffle : ');
        await this.#serviceCall(this.serviceRoon, 'ToggleShuffle');
    }

    async setToggleLoop() {
        logger.info('setToggleLoop : ');
    ...
    async setSourceIndex(index) {
        logger.info('setSourceIndex : ', index);
    ...
    async #serviceCall(service, method, params = null) {
        try {
            ...
            logger.error('serviceCall error : ', error);
            this.manager.emit('renderDeviceError', {
                deviceId: this.id,
                error: error.message,
            });
    ...
    async #updateDeviceInfo() {
        try {
            ...
            logger.error('updateDeviceInfo error : ', error);
            this.manager.emit('renderDeviceError', {
                deviceId: this.id,
                error: error.message,
            });
    ...
    async #updatePlayStatus() {
        try {
            ...
            if (newStatus.sourceName == 'TidalConnect') {
                ...
                logger.info('unknown source : ', newStatus.sourceName);
            }
            ...
            logger.error('updatePlayStatus error : ', error);
            this.manager.emit('renderDeviceError', {
                deviceId: this.id,
                error: error.message,
            });
    ...
    #toInt(value, defaultValue = 0) {
        ...
        if (!isNaN(ret)) {
            ...
            logger.info('toInt value is NaN : ', value);
        }
        return defaultValue;
    }
    ...
    #sourceXMLToObject(xmlString) {
        try {
            ...
            logger.error('sourceXMLToObject error : ', error);
            this.manager.emit('renderDeviceError', {
                deviceId: this.id,
                error: error.message,
            });
    ...
    #trackXMLToObject(xmlString) {
        try {
            ...
            logger.error('trackXMLToObject error : ', error);
            this.manager.emit('renderDeviceError', {
                deviceId: this.id,
                error: error.message,
            });
    ...
    #hardWareInfoXMLToObject(xmlString) {
        try {
            ...
            logger.error('hardWareInfoXMLToObject error : ', error);
            this.manager.emit('renderDeviceError', {
                deviceId: this.id,
                error: error.message,
            });
